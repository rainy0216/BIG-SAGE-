...
# 模型训练
callbacks_list = [
    callbacks.EarlyStopping(monitor='val_mean_squared_error', min_delta=0.1, patience=5, verbose=1, mode='min'),
    callbacks.ModelCheckpoint(filepath='models/' + dataset + '.baseline', monitor='val_mean_squared_error',
                              save_weights_only=True, mode='min', save_best_only=True),
    callbacks.CSVLogger(filename='logs/' + dataset + '_baseline.csv', append=True),
...
sg.utils.plot_history(history, individual_figsize=(8, 5), return_figure=True).savefig(
    'output/' + dataset + '-baseline-metrics.png')
with open('output/' + dataset + '-baseline-metrics.txt', 'w') as json_file:
